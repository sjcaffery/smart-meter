...
- Don't micromanage effort on the main loop - Opus 4.7+/Sonnet 5/Fable 5 already modulate reasoning adaptively per step. Set `effort` explicitly only for a deliberately cheap subagent stage (`low`) or the hardest verify stage (`high`+).
